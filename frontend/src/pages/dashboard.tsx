...
import { useAppointments, formatAppointmentTime } from "@/hooks/use-appointments";
...
export default function Dashboard() {
  ...
  const upcomingAppts = appointments.filter((a) => a.status === "pending" || a.status === "upcoming");
  ...
  if (user?.role === "staff") {
    const staffUpcomingAppointments = appointments
      .filter((a) => a.status === "pending" || a.status === "upcoming")
      .sort((a, b) => `${a.date}T${a.time}`.localeCompare(`${b.date}T${b.time}`));
    const todaysAppointments = staffUpcomingAppointments.filter((a) =>
      a.date?.startsWith(format(new Date(), "yyyy-MM-dd"))
    );
    const listAppointments = todaysAppointments.length > 0 ? todaysAppointments : staffUpcomingAppointments;
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h2 className="font-display font-bold text-lg text-foreground">Today's Appointments</h2>
                  <p className="text-xs text-muted-foreground">Schedule appointments for today</p>
                </div>
              ...
              <div className="mt-4 border-t pt-4">
                ...
                {apptsLoading ? (
                  ...
                  <div className="space-y-2">
                    {listAppointments.slice(0, 3).map((appt) => (
                      <div
                        key={appt.id}
                        className="flex items-center justify-between px-3 py-2 rounded-lg hover:bg-muted/50 transition-colors"
                      >
                        <span className="text-sm font-medium text-foreground">{appt.patientName || "Patient"}</span>
            ...
            >
              <div className="mb-4">
                ...
                <p className="text-xs text-muted-foreground">{latestNews?.source ?? "Updates from the Department of Health"}</p>
              </div>
              ...
              <div className="border-t pt-4 flex-1">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-xs text-muted-foreground">{todaysAppointments.length > 0 ? "Today" : "Upcoming"}</span>
                </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="font-semibold text-sm text-foreground truncate">{appt.patientName || "Patient"}</p>
      ...
      <div className="pb-10">
        ...
        >
          ...
          {specsLoading ? (
            ...
            <p className="text-sm text-muted-foreground text-center py-8">No specializations available</p>
          )}
        </motion.div>
      </div>
    </Layout>
